...
# 페이지 설정
st.set_page_config(page_title="UHNW 가문 통합 자산 시뮬레이터 v18", layout="wide")
...
st.title("🏛️ 가문 통합 자산 시뮬레이터 v18")
st.info("업데이트: Seed/Note 활성화 시 '씨드머니금액'이 실시간으로 계산되어 표시됩니다.")

# ---------------------------------------------------------
# 세션 상태 초기화 (멀티 엔티티 관리)
# ---------------------------------------------------------
if 'app_data_v18' not in st.session_state:
    st.session_state.app_data_v18 = [
        {
...
if st.button("➕ 새 자산 엔티티(신탁) 추가"):
    new_id = len(st.session_state.app_data_v18) + 1
    st.session_state.app_data_v18.append({
    ...
    with st.expander(f"🔹 {entity['name']} 설정 및 비용 분석", expanded=True):
        ...
        with c1:
            ...
            if entity['seed_active']:
                ...
                s_money = b_val * entity['seed_pct']          # 씨드머니금액 계산
                n_p = b_val * (1 - entity['seed_pct'])        # 어음매매금액 계산
                
                st.markdown(f"""
                <div style="background-color:#f8f9fa; padding:10px; border-radius:5px; border:1px solid #ddd;">
                    <p style="margin:0; font-size:14px;">
                        🌱 씨드머니금액: <b>${fmt(s_money)}</b> / 
                        📜 어음매매금액: <b>${fmt(n_p)}</b> / 
                        📅 연간 법적이자: <span style="color:red;">${fmt(n_p * afr_rate)}</span>
                    </p>
                </div>
...
# 시뮬레이션 엔진 및 상세 리포트
# ---------------------------------------------------------
if st.button("🚀 v18 통합 시뮬레이션 실행", type="primary"):
    all_reports = []
    current_assets = [e['amount'] for e in st.session_state.app_data_v18]
    
    # 누적 추적용 변수들
    cum_data = {i: {"intr": 0, "dist": 0, "exp": 0} for i in range(len(st.session_state.app_data_v18))}
